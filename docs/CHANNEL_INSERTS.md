...
| Moj Sint | The loaded patch | Model D, Six-Op PM, Strange Oscillator, Swarm Machine, and Bass Matrix are selectable algorithms within one patch/voice renderer, not five simultaneous outputs. The same boundary applies to schema-8 Dual Filter support; the installation pin still has the five-model/16-start payload. |
