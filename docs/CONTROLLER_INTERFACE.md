...
| FT2 automation | Pattern-owned sparse-lane child. Up/Down changes lanes; REC arms only the selected target and adds/deletes points; POINT browses and adjusts points; LANE changes target, explains fixed RAMP/STEP meaning, and confirms clear. Touch capture uses transport time, can run beside note REC, monitors the armed value, and re-arms pickup on ownership transitions. Back restores exact tracker context. |
