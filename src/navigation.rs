...
const LIVE_PATTERNS: [MenuPage; 4] = [
    page(
        "LAUNCH",
        [
            on("LAUNCH", Action::LiveLaunch),
            on("CANCEL", Action::LiveCancel),
            on("RETRIG", Action::LiveRetrigger),
            on("NOW", Action::LiveLaunchImmediate),
        ],
    ),
    page(
        "TIMING",
        [
            ...
            on("PAT Q", Action::LiveQuantizePattern),
            on("BAR Q", Action::LiveQuantizeBar),
            on("CAPTURE", Action::LiveCaptureToggle),
        ],
    ),
    page(
        "SHAPE",
        [
            on("MUTE", Action::LiveShapeMute),
            on("VEL", Action::LiveShapeVelocity),
            on("GATE", Action::LiveShapeGate),
            on("TRANS", Action::LiveShapeTranspose),
        ],
    ),
    page(
        "SYS",
        [
            on("PANIC", Action::StopAll),
            on("APPEND", Action::LiveCaptureAppend),
            on("REPLACE", Action::LiveCaptureReplace),
            on("EXIT", Action::Back),
        ],
    ),
];
...
const TRACKER_LOOP: [MenuPage; 4] = [
    page(
        "PLAY",
        [
            on("STOP", Action::LoopSlotStop),
            on("LAUNCH", Action::LoopSlotLaunch),
            on("SLOT-", Action::LoopSlotPrevious),
            on("SLOT+", Action::LoopSlotNext),
        ],
    ),
    page(
        "MIX",
        [
            on("LEVEL-", Action::LoopLevelDown),
            on("LEVEL+", Action::LoopLevelUp),
            on("MUTE", Action::LoopSlotMute),
    ...
    page(
        "FILTER",
        [
            on("FILTER-", Action::LoopFilterDown),
            on("FILTER+", Action::LoopFilterUp),
            on("CANCEL", Action::LoopSlotCancel),
...
mod tests {
    ...
    fn transport_is_contextual_and_redundant_variants_are_gone() {
        ...
        let forbidden = ["REC END", "PLAY STOP", "START", "HERE", "TAKE"];
        for screen in Screen::ALL {
            for page in pages(screen, MenuContext::Normal) {
                for slot in page.slots {
                    assert!(
    ...
    fn inventoried_controller_workflow_actions_are_all_reachable() {
        ...
        let inventory = [
            ...
            Action::LoopSlotPrevious,
            Action::LoopSlotNext,
            Action::LoopSlotLaunch,
            Action::LoopSlotStop,
            Action::LoopSlotCancel,
            Action::LoopSlotMute,
            Action::LoopLevelDown,
            Action::LoopLevelUp,
            Action::LoopFilterDown,
            Action::LoopFilterUp,
            Action::LiveLaunch,
            Action::LiveLaunchImmediate,
            Action::LiveCancel,
            Action::LiveRetrigger,
            Action::LiveStop,
            Action::LiveQuantizePattern,
            Action::LiveQuantizeBar,
            Action::LiveCaptureToggle,
