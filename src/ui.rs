...
struct TrackerRecording {
    ...
    lane_owners: HashMap<(usize, usize), u64>,
    next_token: u64,
    notes: usize,
}
...
struct App {
    ...
    help_previous_menu_page: usize,
    help_previous_page_select_mode: bool,
    ...
    loop_activation_seen: u64,
    loop_runtime_pattern: Option<u16>,
    prepared_pattern_loops: Option<PreparedPatternLoops>,
    loop_cycle_start_order: usize,
    ...
    loop_preview_selection: Option<usize>,
    #[cfg(test)]
    loop_preview_load_override: Option<std::result::Result<(), String>>,
    #[cfg(test)]
    loop_runtime_load_override: Option<std::result::Result<(), String>>,
    #[cfg(test)]
    loop_library_directory_override: Option<PathBuf>,
    loop_edit_bars: bool,
...
impl App {
    ...
    fn new(
        ...
        Self {
            ...
            help_previous_menu_page: 0,
            help_previous_page_select_mode: false,
            web_help: None,
            ...
            loop_activation_seen: 0,
            loop_runtime_pattern: None,
            prepared_pattern_loops: None,
            loop_cycle_start_order: 0,
            ...
            loop_preview_selection: None,
            #[cfg(test)]
            loop_preview_load_override: None,
            #[cfg(test)]
            loop_runtime_load_override: None,
            #[cfg(test)]
            loop_library_directory_override: None,
    ...
    fn open_overlay(&mut self, action: Action) {
        ...
        let selection = match kind {
            ...
            OverlayKind::TrackerEntryLayout => {
                self.current_page().map_or(0, |page| match page.entry_mode {
                    sequencer::NoteEntryMode::Manual => 0,
                    sequencer::NoteEntryMode::OneColumn => {
                        1 + usize::from(page.entry_anchor.min(3))
                    }
        ...
        match kind {
            ...
            OverlayKind::TrackerPattern => {
                ...
                if let Some((_, order)) = locations.get(selection).copied() {
                    self.release_tracker_audition();
                    self.select_tracker_order(order);
                    self.close_overlay(false);
                    self.sync_tracker_route();
            ...
            OverlayKind::TrackerSong => {
                if selection < self.song.order.len() {
                    self.release_tracker_audition();
                    self.select_tracker_order(selection);
                    self.close_overlay(false);
                    self.sync_tracker_route();
    ...
    fn install_engine_session(&mut self, mut engine: Engine, session: EngineSession) {
        ...
        let _ = engine.set_mapped_parameters(&session.values);
        ...
        match self.start_engine_process_with_parts(preset, fluid_parts, state) {
            ...
            Err(start_error) => {
                ...
                Err(match restoration {
                    Some(restoration) => format!("{start_error:#} · {restoration}"),
                    None => format!("{start_error:#} · no previous engine"),
                })
            }
        }
    }

    fn recover_failed_in_place_load(&mut self, state: &Path) -> String {
    ...
    fn active_recording_owner(&self) -> Option<RecordingOwner> {
        ...
        } else if self.audio_recorder.status().recording {
    ...
    fn finish_competing_recordings(&mut self, starting: RecordingOwner) {
        ...
        if starting != RecordingOwner::Multitrack && self.audio_recorder.status().recording {
    ...
    fn select_tracker_order(&mut self, order: usize) {
        let old_pattern = self.tracker_pattern_number();
        self.tracker_order = order.min(self.song.order.len().saturating_sub(1));
        if self.tracker_pattern_number() != old_pattern {
            self.loop_slot_selected = 0;
            self.menu_page_by_screen[Screen::TrackerLoop.index()] = 0;
            self.confirm_loop_remove = false;
        }
        self.clamp_tracker_cursor();
    }
    ...
    fn current_loop_settings_mut(&mut self, slot: usize) -> Option<&mut sequencer::LoopSettings> {
        self.current_pattern_mut()?
            .audio_loops
            .get_mut(slot)?
            .as_mut()
    }
    fn current_loop_count(&self) -> usize {
        self.current_pattern()
            .map_or(0, |pattern| pattern.audio_loops.iter().flatten().count())
    }
    ...
    fn write_edit_notes(&mut self, notes: &[(u8, u8)]) {
        ...
        let assignments = match page.entry_mode {
            sequencer::NoteEntryMode::Manual => notes
                .iter()
                .enumerate()
                .map(|(offset, _)| (first_lane + offset) % LANES_PER_PAGE)
                .collect::<Vec<_>>(),
            sequencer::NoteEntryMode::OneColumn => {
                vec![usize::from(page.entry_anchor); notes.len()]
            }
            sequencer::NoteEntryMode::DrumAuto => {
                let Some(assignments) =
                    self.song.patterns.get(&pattern_number).and_then(|pattern| {
                        sequencer::drum_auto_lanes(
                            pattern,
                            row_index,
                            page_index,
                            &notes.iter().map(|(note, _)| *note).collect::<Vec<_>>(),
                            &[],
                        )
                    })
                else {
                    self.status = "DRUM LANES FULL · pattern unchanged".into();
                    return;
                };
                assignments
            }
        ...
        } else if page.entry_mode == sequencer::NoteEntryMode::OneColumn && notes.len() > 1 {
            format!(
                "ONE C{} · chord collapsed · next row {:02X}",
                page.entry_anchor + 1,
                self.tracker_row
            )
    ...
    fn toggle_tracker_page_mute(&mut self) {
        ...
        if let Some(page) = self.current_page_mut() {
            ...
            if muted {
                self.discard_tracker_record_owners(page_index, None);
            }
    ...
    fn toggle_tracker_lane_mute(&mut self) {
        let page_index = self.tracker_page;
        let track = self.tracker_track;
            ...
            .and_then(|page| page.lanes.get_mut(track))
        {
            ...
            self.sequencer.mute(global_lane, muted);
            if muted {
                self.discard_tracker_record_owners(page_index, Some(track));
            }
            self.status = format!("{} {}", name, if muted { "muted" } else { "enabled" });
        }
    }

    fn discard_tracker_record_owners(&mut self, page: usize, lane: Option<usize>) {
        let Some(recording) = self.tracker_recording.as_mut() else {
            return;
        };
        for owners in recording.active_lanes.values_mut() {
            owners
                .retain(|owner| owner.page != page || lane.is_some_and(|lane| owner.lane != lane));
        }
        recording
            .active_lanes
            .retain(|_, owners| !owners.is_empty());
        recording.lane_owners.retain(|(owner_page, owner_lane), _| {
    ...
    fn toggle_tracker_playback(&mut self) {
        ...
        let notes = messages
            ...
            .count();
        let loop_count = self.current_loop_count();
        ...
        match scheduled_software_plan(&messages) {
            ...
            // start one merely because a loop-only Pattern retains that page.
            Ok(None) => {}
            Err(error) => {
        ...
        if loops_ready {
            ...
            self.loop_player.arm_pattern();
            self.loop_runtime_pattern = Some(self.tracker_pattern_number());
        } else {
            self.loop_runtime_pattern = None;
        }
        ...
        if let Some(pattern) = self.song.order.get(incoming_order).copied() {
    ...
    fn toggle_tracker_recording(&mut self) {
        if let Some(return_to_play) = self
            .tracker_recording
            .as_ref()
            .map(|recording| recording.return_to_play)
        {
        ...
        self.tracker_row = if return_to_play {
            transport.row.min(self.tracker_rows().saturating_sub(1))
        } else {
            0
        };
    ...
    fn finish_tracker_recording(&mut self, return_to_play: bool) -> bool {
        ...
        let mut pages = BTreeSet::from([(recording.pattern, recording.page)]);
        pages.extend(
            recording
                .active_lanes
                .values()
                .flatten()
                .map(|active| (active.pattern, active.page)),
        );
        for (pattern_number, page_index) in pages {
    ...
    fn record_tracker_midi_at(&mut self, transport_row: usize, bytes: &[u8]) {
            ...
            let released = self.tracker_recording.as_mut().and_then(|recording| {
                ...
                let released =
                    recording
                        ...
                        .map(|active| {
                            let mut owns_lane =
                                recording.lane_owners.get(&(active.page, active.lane))
                                    == Some(&active.token);
                            if owns_lane {
                                let replacement = recording
                                    .active_lanes
                                    .iter()
                                    .filter(|((_, active_note), _)| *active_note == note)
                                    .flat_map(|(_, owners)| owners)
                                    .filter(|other| {
                                        other.page == active.page && other.lane == active.lane
                                    })
                                    .max_by_key(|other| other.token)
                                    .map(|other| other.token);
                                if let Some(token) = replacement {
                                    recording
                                        .lane_owners
                                        .insert((active.page, active.lane), token);
                                    owns_lane = false;
                                } else {
                                    recording.lane_owners.remove(&(active.page, active.lane));
                                }
                            }
                            (recording.pattern, owns_lane, active)
                        });
            ...
            if let Some((pattern_number, true, active)) = released {
                if let Some(pattern) = self.song.patterns.get_mut(&pattern_number) {
                    ...
                    if rows > 0 {
                        ...
                        if pattern_number == active.pattern && release_row == active.start_row {
                            release_row = (release_row + 1) % rows;
                        }
                        let lane = active.page * LANES_PER_PAGE + active.lane;
                        if let Some(cell) = pattern
                            .rows
                            .get_mut(release_row)
                            .and_then(|row| row.get_mut(lane))
        ...
        let Some(recording) = self.tracker_recording.as_ref() else {
            return;
        };
        let pattern_number = recording.pattern;
        let order = recording.order;
        let page_index = recording.page;
        let next_lane = recording.next_lane;
        let Some(pattern) = self.song.patterns.get(&pattern_number) else {
            return;
        };
        let row = transport_row.min(pattern.rows.len().saturating_sub(1));
        let Some(page) = pattern.pages.get(page_index) else {
            return;
        };
        let first_lane = page_index * LANES_PER_PAGE;
        let mut live_active = recording
            .active_lanes
            .iter()
            .flat_map(|((_, active_note), owners)| {
                owners
                    .iter()
                    .filter(move |active| active.page == page_index)
                    .map(move |active| (active.token, active.lane, *active_note))
            })
            .collect::<Vec<_>>();
        live_active.sort_by_key(|(token, _, _)| *token);
        let live_active = live_active
            .into_iter()
            .map(|(_, lane, active_note)| (lane, active_note))
            .collect::<Vec<_>>();
        let lane = match page.entry_mode {
            sequencer::NoteEntryMode::Manual => (0..LANES_PER_PAGE)
                .map(|offset| (next_lane + offset) % LANES_PER_PAGE)
                .find(|lane| {
                    !live_active
                        .iter()
                        .any(|(active_lane, _)| active_lane == lane)
            ...
            sequencer::NoteEntryMode::OneColumn => Some(usize::from(page.entry_anchor)),
            sequencer::NoteEntryMode::DrumAuto => {
                sequencer::drum_auto_lanes(pattern, row, page_index, &[note], &live_active)
                    .and_then(|lanes| lanes.into_iter().next())
            }
        };
        let Some(lane) = lane else {
            self.status = if page.entry_mode == sequencer::NoteEntryMode::DrumAuto {
                "DRUM LANES FULL · pattern unchanged".into()
            } else {
                format!("REC row {row:02X} full · note ignored")
            };
            return;
        };
        let Some(pattern) = self.song.patterns.get_mut(&pattern_number) else {
        ...
        let token = recording.next_token;
        recording.next_token = recording.next_token.wrapping_add(1).max(1);
        recording.lane_owners.insert((page_index, lane), token);
        recording
            .active_lanes
            .entry((channel, note))
            .or_default()
    ...
    fn refresh_tracker_record_loop(&self) {
        if let Some(recording) = self
            .tracker_recording
            .as_ref()
            .filter(|recording| !recording.return_to_play)
        {
            self.sequencer
    ...
    fn retrigger_live_pattern(&mut self) {
        ...
        let requires_engine_prepare = plan
            .as_ref()
            .is_some_and(|plan| !self.live_software_plan_ready(plan));
        self.stage_pattern_loops(queued.pattern);
        self.sequencer
            .live_queue(&song, queued, requires_engine_prepare);
        self.status = format!(
    ...
    fn load_current_pattern_loops(&mut self) -> (Vec<usize>, bool) {
        #[cfg(test)]
        if self.loop_runtime_load_override.is_some() {
            return (self.load_current_pattern_loops_with_test_override(), true);
        }
        let pattern_number = self.tracker_pattern_number();
        let Some(prepared) = self.prepare_pattern_loops(pattern_number) else {
            return ((0..crate::loop_player::LOOP_SLOTS).collect(), false);
        };
        ...
        let (failed, ready) = match result {
            Ok(failed_slots) => (
                failed_slots
                    .iter()
                    .enumerate()
                    .filter_map(|(slot, failed)| failed.then_some(slot))
                    .collect(),
                true,
            ),
            Err(_) => ((0..crate::loop_player::LOOP_SLOTS).collect(), false),
        };
        if !stable_route {
            self.retry_final_bus();
        }
    ...
    fn load_preview_loops(&mut self, song: &Song) -> Vec<usize> {
        ...
        let tempo = pattern.map_or(self.config.external_midi.default_tempo, |pattern| {
            pattern.tempo
        });
        let meter = pattern.map_or(4, |pattern| pattern.meter);
        let mut failed = Vec::new();
        for slot in 0..crate::loop_player::LOOP_SLOTS {
            self.loop_player.unload_slot(slot);
            let Some(settings) = pattern.and_then(|pattern| pattern.audio_loops[slot].as_ref())
            else {
                continue;
            };
    ...
    fn loop_library_directory(&self) -> PathBuf {
        #[cfg(test)]
        if let Some(path) = self.loop_library_directory_override.as_ref() {
            return path.clone();
        }
    ...
    fn load_loop_settings_for_slot(
        ...
        self.load_loop_settings_for_slot_at(
            slot,
            settings,
            self.current_tempo(),
            self.current_meter(),
    ...
    fn load_loop_settings_for_slot_at(
        &mut self,
        slot: usize,
        settings: Option<sequencer::LoopSettings>,
        tempo: u16,
    ...
    fn activate_pattern_loops(&mut self, pattern_number: u16) -> Vec<usize> {
        let prepared = self
            .prepared_pattern_loops
            .take()
            .filter(|prepared| prepared.pattern == pattern_number)
            .or_else(|| self.prepare_pattern_loops(pattern_number));
        let Some(prepared) = prepared else {
            self.loop_player.stop();
            self.loop_runtime_pattern = None;
            return (0..crate::loop_player::LOOP_SLOTS).collect();
        };
        let selected = self.loop_slot_selected;
        let stable_route = self.loop_player.backend_active();
        if !stable_route {
        ...
        self.loop_player.arm_pattern();
        self.loop_slot_selected = selected;
        self.loop_runtime_pattern = Some(pattern_number);
        if !stable_route {
            self.retry_final_bus();
        }
        failed
    ...
    fn prepare_pattern_loops(&self, pattern_number: u16) -> Option<PreparedPatternLoops> {
        let pattern = self.song.patterns.get(&pattern_number)?;
        let mut slots = std::array::from_fn(|_| None);
        let mut faults = std::array::from_fn(|_| None);
        let directory = self.loop_library_directory();
        for slot in 0..crate::loop_player::LOOP_SLOTS {
            let Some(settings) = pattern.audio_loops[slot].clone() else {
                continue;
            };
            match crate::loop_player::DecodedLoop::open(&directory.join(&settings.file)) {
                Ok(decoded) => slots[slot] = Some((decoded, settings)),
                Err(error) => faults[slot] = Some((settings.file, error.to_string())),
            }
        }
        Some(PreparedPatternLoops {
            pattern: pattern_number,
            tempo: pattern.tempo,
            meter: pattern.meter,
            slots,
    ...
    fn stage_pattern_loops(&mut self, pattern_number: u16) {
        self.prepared_pattern_loops = self.prepare_pattern_loops(pattern_number);
    }

    fn loop_editor_can_touch_runtime(&self) -> bool {
        self.loop_runtime_pattern.map_or_else(
            || !self.sequencer.status().playing,
            |owner| owner == self.tracker_pattern_number(),
        )
    }
    ...
    fn command_loop_slot(&mut self, command: crate::loop_player::LoopCommand) {
        let slot = self.loop_slot_selected;
        if !self.loop_editor_can_touch_runtime() {
            self.status = "selected Pattern is not sounding".into();
            return;
        }
        if self.current_loop_settings(slot).is_none() {
    ...
    fn cancel_loop_slot_queue(&mut self) {
        if !self.loop_editor_can_touch_runtime() {
            self.status = "selected Pattern has no runtime queue".into();
            return;
        }
    ...
    fn remove_pattern_loop(&mut self) {
        let slot = self.loop_slot_selected;
        if self.current_loop_settings(slot).is_none() {
            self.confirm_loop_remove = false;
            self.status = "Pattern has no loop".into();
            return;
        }
    ...
    fn selected_loop_preview(&self) -> Option<(PathBuf, String)> {
        let selection = self.overlay.as_ref()?.selection;
        if let Some(path) = self.loop_imports.get(selection) {
            let label = path
                .file_name()
                .and_then(|name| name.to_str())
                .unwrap_or("inbox loop")
                .to_owned();
            return Some((path.clone(), label));
        }
        let entry = self.loop_library.get(selection - self.loop_imports.len())?;
        Some((
            self.loop_library_directory().join(&entry.file),
            entry.file.clone(),
        ))
    }

    fn load_loop_preview_candidate(
        &mut self,
        decoded: crate::loop_player::DecodedLoop,
        settings: &sequencer::LoopSettings,
    ) -> Result<()> {
        #[cfg(test)]
        if let Some(result) = self.loop_preview_load_override.as_ref() {
            result.clone().map_err(anyhow::Error::msg)?;
            self.loop_player
                .set_preview_status(crate::loop_player::LoopStatus {
                    loaded: true,
    ...
    fn toggle_loop_preview(&mut self) {
        let Some(selection) = self.overlay.as_ref().and_then(|overlay| {
            (overlay.kind == OverlayKind::LoopLibrary).then_some(overlay.selection)
        }) else {
            return;
        };
        ...
        self.stop_loop_preview(false);
        let Some((path, label)) = self.selected_loop_preview() else {
            self.status = "no loop selected for preview".into();
            return;
        };
        ...
        if let Some(pattern) = preview.patterns.get_mut(&0) {
            pattern.tempo = self.current_tempo();
        }
        self.sequencer.play(&preview, 0, 0);
        let _ = self
            .loop_player
            .command_slot_immediate(0, crate::loop_player::LoopCommand::Launch);
        self.loop_player.arm_pattern();
    ...
    fn refresh_loop_library(&mut self) {
        match crate::loop_player::library_entries(
            ...
            self.current_loop_settings(self.loop_slot_selected),
            &sequencer::songs_dir(),
        ) {
            Ok(entries) => {
    ...
    fn select_loop_library_entry(&mut self, index: usize) -> bool {
    ...
    fn auto_align_loop(&mut self) {
        let Some(settings) = self.current_loop_settings(self.loop_slot_selected).cloned() else {
    ...
    fn adjust_loop_source_bpm(&mut self, direction: i8) {
        let tempo = if let Some(settings) = self.current_loop_settings_mut(self.loop_slot_selected)
        {
            ...
            Some(Self::loop_pattern_tempo(settings))
        } else {
            None
        };
    ...
    fn load_song_named(&mut self, name: &str) {
        ...
        match sequencer::load(&sequencer::songs_dir(), name) {
            Ok(mut song) => {
                ...
                self.refresh_page_targets();
                self.sync_tracker_route();
                let (failed, _) = self.load_current_pattern_loops();
                if failed.is_empty() {
                    self.status.clear();
                } else {
                    self.status = format!(
    ...
    fn pattern_setup_status(&mut self) {
        let loop_note = if !self.pattern_setup_new && self.current_loop_count() > 0 {
            format!(" · {} loops detach", self.current_loop_count())
        } else {
            String::new()
        };
        self.status = format!(
            "{} pattern · {}/4 · {} rows{loop_note} · confirm",
    ...
    fn create_pattern(&mut self, rows: usize) {
        ...
        self.create_pattern_from(pattern);
    }
    ...
    fn create_pattern_from(&mut self, pattern: sequencer::Pattern) {
        let rows = pattern.rows.len();
        ...
        self.sync_tracker_route();
        self.status = format!(
            "new pattern {number} · {meter}/4 · {rows} rows · order {:02}/{:02}",
    ...
    fn tracker_note_off(&mut self) {
        ...
        self.cancel_tracker_gesture();
        let lane = self.current_page().map_or(self.tracker_track, |page| {
            if page.entry_mode == sequencer::NoteEntryMode::OneColumn {
                usize::from(page.entry_anchor)
            } else {
                self.tracker_track
            }
        });
    ...
    fn open_help(&mut self) {
        if self.screen != Screen::Help {
            ...
            self.help_previous_menu_page = self.menu_page();
    ...
    fn close_help(&mut self) {
        ...
        let previous = self.help_previous;
    ...
    fn load_idea(&mut self, state: &Path, _tx: std::sync::mpsc::Sender<MidiEvent>) {
        ...
        match recording::load_with_parameters(&recording::ideas_dir(), &name) {
            Ok((preset, saved_values, events)) => {
                ...
                match self.replace_engine_process(&preset, EngineOwner::SoftwareSynth, state) {
                    Ok(audio_route) => {
                        self.stop_recording();
                        self.stop_playback();
                        self.finish_idea_load(
    ...
    fn tick(&mut self) {
        ...
        if self.screen == Screen::Tracker {
            ...
            if tracker.playing && !tracker.available {
                ...
                if tracker.error.is_some() {
                    ...
                    self.status = "FT2 TARGET OFFLINE · recording stopped".into();
                }
            }
        ...
        {
            self.loop_activation_seen = tracker.loop_activation_serial;
            if let Some(pattern) = tracker.loop_pattern {
                let failed = self.activate_pattern_loops(pattern);
                if let Some(order) = tracker.loop_order {
                    let incoming_order = if order + 1 < self.song.order.len() {
                        order + 1
                    } else {
                        self.loop_cycle_start_order
                    };
                    if let Some(incoming) = self.song.order.get(incoming_order).copied() {
                        self.stage_pattern_loops(incoming);
                    }
                }
                if !failed.is_empty() {
                    self.status = format!(
                        "{} Loop Mix slot(s) faulted · MIDI and healthy slots continue",
                        failed.len()
                    );
                }
            } else {
...
fn perform(
    ...
    if a.screen == Screen::TrackerFiles && a.confirm_pattern_clear {
        match action {
            ...
            Action::Back => {
                let operation = if a.pattern_setup_new { "new" } else { "clear" };
                a.confirm_pattern_clear = false;
                a.reset_context_page();
                a.status = format!("pattern {operation} cancelled");
            }
            _ => {}
        }
        return false;
...
fn key(code: KeyCode, a: &mut App, state: &Path, tx: &std::sync::mpsc::Sender<MidiEvent>) -> bool {
    ...
    if a.screen == Screen::Tracker {
        ...
        if a.note_editor.is_some() {
            let action = match code {
                KeyCode::Char('-') => Some(Action::NoteEditorDecrease),
                KeyCode::Char('+') | KeyCode::Char('=') => Some(Action::NoteEditorIncrease),
        ...
        match code {
            ...
            KeyCode::PageUp => {
                a.cancel_tracker_gesture();
                a.select_tracker_order(a.tracker_order.saturating_sub(1));
            ...
            KeyCode::PageDown => {
                a.cancel_tracker_gesture();
                a.select_tracker_order(
                    (a.tracker_order + 1).min(a.song.order.len().saturating_sub(1)),
                );
                if !a.leave_noob_on_percussion() {
                    a.sync_tracker_route();
                }
...
fn draw_live_patterns<B: Backend>(f: &mut Frame<B>, a: &App) {
    ...
    for offset in 0..4 {
        ...
        let loop_count = pattern.audio_loops.iter().flatten().count();
        let text = format!(
            "{} P{pattern_number:02} {}{} {:>3}r {:>3} BPM L{loop_count}",
            if selected { ">" } else { " " },
...
fn draw_tracker_loop<B: Backend>(f: &mut Frame<B>, a: &mut App) {
    ...
    let mut lines = vec![Spans::from(Span::styled(
        format!(
            "PAT {:02} LOOP MIX · FOUR WAV SLOTS",
    ...
    for slot in 0..crate::loop_player::LOOP_SLOTS {
        ...
        let settings = a.current_loop_settings(slot);
    ...
    if let Some(settings) = a.current_loop_settings(a.loop_slot_selected) {
...
fn draw_tracker_loop_align<B: Backend>(f: &mut Frame<B>, a: &App) {
    let z = f.size();
    let details = if let Some(settings) = a.current_loop_settings(a.loop_slot_selected) {
        let bar_unit = i32::from(a.current_meter().clamp(1, 16));
        let offset_bars = f64::from(settings.offset_beats) / f64::from(bar_unit);
        format!(
...
fn overlay_rows(a: &App, overlay: &OverlayState) -> Vec<String> {
    match overlay.kind {
        ...
        OverlayKind::TrackerAdvance => (0..=32)
            ...
            .collect(),
        OverlayKind::TrackerEntryLayout => vec![
            "MANUAL · selected column".into(),
            "ONE COLUMN · anchor C1".into(),
            "ONE COLUMN · anchor C2".into(),
...
fn draw_overlay_launcher<B: Backend>(f: &mut Frame<B>, a: &mut App) {
    let Some(overlay) = a.overlay.as_ref().cloned() else {
        return;
    };
    let z = f.size();
    ...
    for item in 0..4 {
        ...
        f.render_widget(
            Paragraph::new(format!(
                "[{}]",
                truncate(label, usize::from(button.width.saturating_sub(2)))
            ))
            .alignment(Alignment::Center)
            .style(
                Style::default()
    ...
    let setup_label = a
        ...
        .map(|column| {
            ...
            format!(
                ...
                a.current_page().map_or("MANUAL", |page| {
                    page.entry_mode.compact_label(page.entry_anchor)
                })
            )
        })
...
fn draw_tracker_files<B: Backend>(f: &mut Frame<B>, a: &mut App) {
    ...
    if a.confirm_pattern_clear {
        let attached = a.current_loop_count();
        let lines = vec![
            ...
            Spans::from(if !a.pattern_setup_new && attached > 0 {
                format!("DETACH ▶ {attached} Loop Mix slot(s)")
            } else {
                String::new()
            }),
...
impl ScreenshotSpecialScenario {
    ...
    const fn slug(self) -> &'static str {
        match self {
            ...
            Self::LoopEmpty => "loop-pattern-empty",
            Self::LoopStates => "loop-slot-states",
            Self::PatternClearLoops => "pattern-clear-attached-loops",
            Self::LiveLoopSwitch => "live-pattern-loop-switch",
            Self::LoopCompact => "loop-compact",
...
fn configure_screenshot(app: &mut App, screen: Screen) {
    ...
    match screen {
        ...
        Screen::TrackerLoop => {
            ...
            pattern.audio_loops[0] = Some(settings.clone());
            pattern.audio_loops[1] = Some(sequencer::LoopSettings {
                file: "bass-stem.wav".into(),
                level_x1000: 850,
                filter_x1000: -250,
                ..settings.clone()
            });
            pattern.audio_loops[2] = Some(sequencer::LoopSettings {
            ...
            pattern.audio_loops[3] = Some(sequencer::LoopSettings {
    ...
    match scenario {
        ...
        ScreenshotScenario::TrackerLoopAlign => {
            ...
            if let Some(settings) = app
                ...
                .patterns
                .get_mut(&0)
                .and_then(|pattern| pattern.audio_loops[0].as_mut())
            {
...
fn configure_special_screenshot_scenario(app: &mut App, scenario: ScreenshotSpecialScenario) {
    match scenario {
        ...
        ScreenshotSpecialScenario::LoopPatternA => {
            configure_screenshot_scenario(app, ScreenshotScenario::TrackerLoop);
            app.status = "PAT 00 Loop Mix · sounding".into();
        }
        ScreenshotSpecialScenario::LoopPatternB => {
            configure_screenshot_scenario(app, ScreenshotScenario::TrackerLoop);
            ...
            pattern.audio_loops[2] = Some(sequencer::LoopSettings::new(
                "pattern-b-texture.wav".into(),
                9_600,
                sequencer::BpmInterpretation::Normal,
                4,
                8,
            ...
            app.loop_player.set_slot_preview_status(
                ...
                crate::loop_player::LoopStatus {
                    ...
                    running: true,
                    playing: true,
                    file: Some("pattern-b-drums.wav".into()),
                    source_rate: 48_000,
                    source_channels: 2,
                    ..crate::loop_player::LoopStatus::default()
                },
            ...
            app.loop_player.set_slot_preview_status(
                ...
                crate::loop_player::LoopStatus {
                    loaded: true,
                    file: Some("pattern-b-texture.wav".into()),
                    ..crate::loop_player::LoopStatus::default()
                },
            );
            app.status = "PAT 01 Loop Mix · Pattern A still sounds while browsing".into();
        }
        ScreenshotSpecialScenario::LoopEmpty => {
            configure_screenshot_scenario(app, ScreenshotScenario::TrackerLoop);
            app.current_pattern_mut().unwrap().audio_loops = std::array::from_fn(|_| None);
            for slot in 0..crate::loop_player::LOOP_SLOTS {
                app.loop_player
                    .set_slot_preview_status(slot, crate::loop_player::LoopStatus::default());
            }
            app.status = "PAT 00 Loop Mix · empty".into();
        }
        ScreenshotSpecialScenario::LoopStates => {
            configure_screenshot_scenario(app, ScreenshotScenario::TrackerLoop);
            app.loop_player.set_slot_preview_status(
                0,
                crate::loop_player::LoopStatus {
            ...
            app.loop_player.set_slot_preview_status(
                ...
                crate::loop_player::LoopStatus {
                    loaded: true,
                    file: Some("bass-stem.wav".into()),
                    error: Some("loop tempo no longer matches Pattern tempo".into()),
                    ..crate::loop_player::LoopStatus::default()
                },
            );
            app.loop_player.set_slot_preview_status(
                ...
                crate::loop_player::LoopStatus {
                    ...
                    running: true,
                    file: Some("pads-8-bars.wav".into()),
                    queued: Some(crate::loop_player::LoopCommand::Stop),
                    ..crate::loop_player::LoopStatus::default()
                },
            );
            app.current_pattern_mut().unwrap().audio_loops[3] = None;
            app.loop_player.set_slot_preview_status(
                3,
                crate::loop_player::LoopStatus {
                    error: Some("private WAV missing".into()),
            ...
            app.status = "Stopped · incompatible · queued stop · missing".into();
        }
        ScreenshotSpecialScenario::PatternClearLoops => {
            configure_screenshot_scenario(app, ScreenshotScenario::TrackerLoop);
            app.screen = Screen::TrackerFiles;
            app.choose_pattern_clear();
        }
        ScreenshotSpecialScenario::LiveLoopSwitch => {
            configure_screenshot_scenario(app, ScreenshotScenario::LivePatterns);
            ...
            app.status = "PAT 01 queued · Loop Mix prepared · PAT boundary".into();
        }
        ScreenshotSpecialScenario::LoopCompact => {
            configure_screenshot_scenario(app, ScreenshotScenario::TrackerLoop);
...
mod tests {
    ...
    fn write_test_loop(path: &Path) {
        ...
        for frame in 0..8_000 {
            let sample = if frame % 2_000 < 80 { 16_000 } else { 0 };
    ...
    #[test]
    fn loop_overlay_places_stop_play_and_launcher_inside_border_above_status() {
        let p = presets();
        let mut a = app(&p);
        ...
        let actions = a
            ...
            .actions
            .iter()
            .map(|(area, action)| (area.y, *action))
            .collect::<Vec<_>>();

        assert!(row_text(&buffer, 11).contains("STOP"));
        assert!(row_text(&buffer, 11).contains("PLAY"));
        assert!(row_text(&buffer, 11).contains("LIBRARY"));
        assert_eq!(
            actions,
            [
                (11, Action::LoopPreviewStop),
                (11, Action::LoopPreview),
                (11, Action::OpenLoopLibrary),
            ]
        );
        assert!(row_text(&buffer, 12).starts_with(transport_glyph(a.transport_indicator()).0));
    }
    ...
    #[test]
    fn loop_browser_preview_is_explicit_repeatable_and_stops_on_every_leave_path() {
        ...
        write_test_loop(&first);
        write_test_loop(&second);
        ...
        a.tracker_page = 1;
        a.tracker_track = 3;
        a.config.loop_player.import_directory = base.clone();
        a.loop_imports = vec![first, second];
        a.loop_preview_load_override = Some(Ok(()));
        let project = a.song.clone();
        a.open_overlay(Action::OpenLoopLibrary);
        assert!(!a.loop_previewing, "selection alone must stay silent");

        a.toggle_loop_preview();
        assert!(a.loop_previewing);
        assert_eq!(a.loop_preview_selection, Some(0));
        assert_eq!(a.song, project, "preview must not attach the loop");
        assert_eq!(
            (
                a.tracker_order,
                a.tracker_row,
        ...
        a.move_overlay(1);
        assert!(!a.loop_previewing, "selection change must stop preview");
        assert_eq!(a.overlay.as_ref().unwrap().selection, 1);
        a.toggle_loop_preview();
        assert!(a.loop_previewing);
        a.toggle_loop_preview();
        assert!(!a.loop_previewing, "repeated PLAY toggles preview off");

        a.toggle_loop_preview();
        a.close_overlay(true);
        assert!(!a.loop_previewing, "Back/close must stop preview");
        assert_eq!(a.screen, Screen::TrackerLoop);

        a.open_overlay(Action::OpenLoopLibrary);
        a.toggle_loop_preview();
        a.set_screen(Screen::Tracker);
        assert!(!a.loop_previewing, "leaving Loop Player must stop preview");
    ...
    #[test]
    fn failed_loop_preview_and_import_retain_selection_and_rollback_private_copy() {
        let base = std::env::temp_dir().join(format!(
            "shr-loop-rollback-{}-{}",
            std::process::id(),
            SystemTime::now()
        ...
        let source = inbox.join("retry.wav");
        write_test_loop(&source);

        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::TrackerLoop;
        a.config.loop_player.import_directory = inbox.clone();
        a.loop_imports = vec![source];
        a.loop_library_directory_override = Some(library.clone());
        a.loop_preview_load_override = Some(Err("preview injection".into()));
        a.open_overlay(Action::OpenLoopLibrary);
        a.toggle_loop_preview();
        assert!(!a.loop_previewing);
    ...
    #[test]
    fn loop_import_completes_once_per_activation_and_repeated_import_stays_consistent() {
        let base = std::env::temp_dir().join(format!(
            "shr-loop-import-success-{}-{}",
            std::process::id(),
            SystemTime::now()
                .duration_since(UNIX_EPOCH)
                .unwrap()
        ...
        fs::create_dir_all(&inbox).unwrap();
        fs::create_dir_all(&library).unwrap();
        let source = inbox.join("repeat.wav");
        write_test_loop(&source);
    ...
    #[test]
    fn ideas_enter_encoder_and_mouse_all_inspect_while_load_stays_explicit() {
        let p = presets();
        let (tx, _rx) = mpsc::channel();
        let setup = || {
            let mut a = app(&p);
            a.screen = Screen::Ideas;
            a.controller_layout = ControllerLayout::Eight;
            a.ideas = vec!["missing-idea".into()];
            a
        };
        ...
        mouse(
            MouseEvent {
                kind: MouseEventKind::Down(MouseButton::Left),
                column: pointer.hits.list.x,
                row: pointer.hits.list.y,
                modifiers: crossterm::event::KeyModifiers::NONE,
            },
            &mut pointer,
            Path::new("/none"),
    ...
    #[test]
    fn routing_failures_retain_the_entered_draft_row_and_retry_context() {
        let p = presets();
        let mut invalid = app(&p);
        invalid.screen = Screen::Routing;
        invalid.routing.selected = 4;
        invalid.begin_routing_edit();
        invalid
            .routing
            .draft
            .as_mut()
            .unwrap()
            .config
            .midi_performance_input_matches = vec!["Keys".into(), "Keys".into()];
        ...
        fs::write(&missing_parent, b"not a directory").unwrap();
        let mut save_failure = app(&p);
        save_failure.screen = Screen::Routing;
        save_failure.routing.selected = 2;
        save_failure.begin_routing_edit();
        save_failure
            .routing
            .draft
            .as_mut()
        ...
        let expected_layout = save_failure
            .routing
            .draft
            .as_ref()
            .unwrap()
            .controller
            .layout;
        save_failure.confirm_routing_edit(&missing_parent);
        let retained = save_failure.routing.draft.as_ref().unwrap();
    ...
    fn ft2_record_rotary_ignores_turns_until_every_recorded_note_is_off() {
        ...
        a.tracker_recording = Some(TrackerRecording {
            ...
            lane_owners: HashMap::from([((0, LANES_PER_PAGE - 1), 1)]),
            next_token: 2,
            notes: 1,
        });
    ...
    #[test]
    fn lowercase_tracker_note_keys_do_not_become_record_stop_or_back_shortcuts() {
        let p = presets();
        let (tx, _rx) = mpsc::channel();
        let mut a = app(&p);
        connect_test_midi_hardware(&mut a);
        ...
        key(KeyCode::Char('s'), &mut a, Path::new("/none"), &tx);
        key(KeyCode::Char('b'), &mut a, Path::new("/none"), &tx);
    ...
    #[test]
    fn exclusive_recording_switches_owners_and_failed_preflight_keeps_current_owner() {
        let p = presets();
        let mut a = app(&p);
        a.engine = Some(
        ...
        assert_eq!(
            ...
            Some(RecordingOwner::Idea),
            "failed replacement preflight must not discard the active capture"
        );

        a.available_page_outputs = vec!["Test DIN".into()];
        a.toggle_tracker_recording();
        ...
        assert_eq!(
            a.active_recording_owner(),
            Some(RecordingOwner::Tracker),
            "failed Idea preflight must keep FT2 recording active"
        );
        a.engine = Some(
            Engine::start_test_process(BackendKind::Synthv1, Arc::clone(&a.midi_output)).unwrap(),
        );
        ...
        assert_eq!(
            a.active_recording_owner(),
            None,
            "repeated REC stops its owner"
        );

        a.toggle_idea_recording();
        a.set_screen(Screen::Ideas);
        perform(Action::Back, &mut a, Path::new("/none"), None);
        assert_eq!(a.screen, Screen::Home);
        assert_eq!(a.active_recording_owner(), Some(RecordingOwner::Idea));
        assert_eq!(
    ...
    #[test]
    fn help_preserves_and_restores_the_exact_active_tracker_context() {
        let p = presets();
        let mut a = app(&p);
        connect_test_midi_hardware(&mut a);
        a.set_screen(Screen::Tracker);
        ...
        let recording = a
            .tracker_recording
            .as_ref()
            .map(|recording| (recording.pattern, recording.order, recording.page));
        let song = a.song.clone();
        ...
        assert_eq!(a.screen, Screen::Help);
        assert_eq!(
            a.tracker_recording.as_ref().map(|recording| (
                recording.pattern,
                recording.order,
                recording.page
            )),
    ...
    #[test]
    fn help_preserves_an_active_cell_editor_draft_and_cursor() {
        let p = presets();
        let mut a = app(&p);
        a.set_screen(Screen::Tracker);
        a.tracker_row = 5;
        ...
        let field = a.note_editor.as_ref().unwrap().field;
        let active = a.note_editor.as_ref().unwrap().active;

        a.open_help();
        a.close_help();

        assert_eq!(a.screen, Screen::Tracker);
        assert_eq!((a.tracker_row, a.tracker_page, a.tracker_track), (5, 1, 2));
        assert_eq!(a.note_editor.as_ref().unwrap().draft, draft);
        assert_eq!(a.note_editor.as_ref().unwrap().field, field);
        assert_eq!(a.note_editor.as_ref().unwrap().active, active);
    }
    ...
    fn tracker_play_record_and_edit_modes_are_mutually_exclusive() {
        ...
        assert_eq!(a.tracker_mode, TrackerMode::Play);
        assert!(a.sequencer.status().playing);
        assert!(a.status.contains("REC punch-out"));
    ...
    #[test]
    fn loop_removal_is_confirmed_and_only_clears_the_pattern_reference() {
        let p = presets();
        let mut a = app(&p);
        a.song.patterns.get_mut(&0).unwrap().audio_loops[0] = Some(sequencer::LoopSettings {
        ...
        assert!(a.current_loop_settings(0).is_some());
        perform(Action::Up, &mut a, Path::new("/none"), None);
        assert!(!a.confirm_loop_remove);
        assert!(a.current_loop_settings(0).is_some());
        assert!(
    ...
    fn loop_removal_stops_only_the_selected_slot() {
        ...
        pattern.audio_loops[0] = Some(settings.clone());
        pattern.audio_loops[3] = Some(settings);
        for slot in [0, 3] {
        ...
        assert!(a.current_loop_settings(0).is_some());
        assert!(a.current_loop_settings(3).is_none());
    }
    ...
    #[test]
    fn browsing_patterns_changes_loop_editor_owner_without_changing_sounding_owner() {
        let p = presets();
        let mut a = app(&p);
        let first = sequencer::LoopSettings::new(
            "pattern-a.wav".into(),
            12_000,
            sequencer::BpmInterpretation::Normal,
            0,
        ...
        a.song.patterns.get_mut(&0).unwrap().audio_loops[0] = Some(first);
        let mut second = sequencer::Pattern::empty_like_setup(32, &a.song.patterns[&0]);
        second.audio_loops[2] = Some(sequencer::LoopSettings::new(
            "pattern-b.wav".into(),
            12_000,
            sequencer::BpmInterpretation::Normal,
            0,
        ...
        a.song.append_pattern(second).unwrap();
        a.loop_runtime_pattern = Some(0);
        a.loop_slot_selected = 3;
        a.loop_player.set_slot_preview_status(
            2,
            crate::loop_player::LoopStatus {
                loaded: true,
                running: true,
                file: Some("sounding-a.wav".into()),
                ..crate::loop_player::LoopStatus::default()
            },
        );

        a.select_tracker_order(1);
        assert_eq!(a.tracker_pattern_number(), 1);
        assert_eq!(a.loop_slot_selected, 0);
        assert_eq!(
            a.loop_runtime_pattern,
            Some(0),
            "editing browse must not change sounding Pattern"
        );
        assert!(a.current_loop_settings(0).is_none());
        assert_eq!(
            a.current_loop_settings(2)
                .map(|settings| settings.file.as_str()),
            Some("pattern-b.wav")
        );
        a.loop_slot_selected = 2;
        a.adjust_loop_slot_level(-1);
        a.adjust_loop_slot_filter(1);
        assert_eq!(
            a.current_loop_settings(2)
                .map(|settings| (settings.level_x1000, settings.filter_x1000)),
            Some((950, 50))
        );
        assert_eq!(
            a.loop_player.slot_control_values(2),
            (1000, 0),
            "editing a browsed Pattern must not touch sounding controls"
        );
        a.command_loop_slot(crate::loop_player::LoopCommand::Launch);
        assert_eq!(a.loop_player.slot_status(2).queued, None);
        assert_eq!(a.status, "selected Pattern is not sounding");

        a.confirm_loop_remove = true;
        a.remove_pattern_loop();
        assert!(a.current_loop_settings(2).is_none());
        assert!(
            a.loop_player.slot_status(2).loaded,
            "detaching from a browsed Pattern must not unload the sounding slot"
        );
    }

    #[test]
    fn live_queue_preparation_is_replaced_and_cancelled_as_one_pattern_unit() {
        let p = presets();
        let mut a = app(&p);
        let second = a
            .song
            .append_pattern(sequencer::Pattern::empty_like_setup(
                16,
                &a.song.patterns[&0],
            ))
            .unwrap();
        a.stage_pattern_loops(0);
        assert_eq!(
            a.prepared_pattern_loops
                .as_ref()
                .map(|prepared| prepared.pattern),
            Some(0)
        );
        a.stage_pattern_loops(second);
        assert_eq!(
            a.prepared_pattern_loops
                .as_ref()
                .map(|prepared| prepared.pattern),
        ...
        a.cancel_live_queue();
        assert!(a.prepared_pattern_loops.is_none());
    }
    ...
    #[test]
    fn page_entry_overlay_selects_one_column_anchor_without_moving_cursor() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        a.tracker_track = 3;
        ...
        assert_eq!(
            a.overlay.as_ref().map(|overlay| overlay.kind),
            Some(OverlayKind::TrackerEntryLayout)
        );
        a.overlay.as_mut().unwrap().selection = 2;
        a.activate_overlay();
        let page = a.current_page().unwrap();
        assert_eq!(page.entry_mode, sequencer::NoteEntryMode::OneColumn);
        assert_eq!(page.entry_anchor, 1);
        assert_eq!(a.tracker_track, 3);
        assert_eq!(a.status, "PAGE ENTRY · ONE C2");
    }
    ...
    fn manual_and_one_column_step_entry_keep_literal_monophonic_lanes() {
        ...
        a.set_tracker_edit(true);
        a.tracker_page = a.percussion_page_index().unwrap();
        a.current_page_mut().unwrap().entry_mode = sequencer::NoteEntryMode::Manual;
        a.tracker_track = 3;
        a.tracker_single_note(36, 100);
        let page_start = a.tracker_page * LANES_PER_PAGE;
        assert_eq!(
            a.song.patterns[&0].rows[0][page_start + 3].note,
            Note::On(36)
        );
        ...
        a.tracker_track = 0;
        a.tracker_row = 0;
        let page = a.current_page_mut().unwrap();
        page.entry_mode = sequencer::NoteEntryMode::OneColumn;
        page.entry_anchor = 2;
        for note in [40, 43, 47, 50] {
            a.tracker_single_note(note, 96);
        }
        for (row, note) in [40, 43, 47, 50].into_iter().enumerate() {
            let cells = &a.song.patterns[&0].rows[row][..LANES_PER_PAGE];
            assert_eq!(cells[2].note, Note::On(note));
            assert!(cells
                .iter()
                .enumerate()
                .all(|(lane, cell)| lane == 2 || cell.note == Note::Empty));
        }
        assert_eq!(a.tracker_track, 0);
        a.tracker_note_off();
        assert_eq!(a.song.patterns[&0].rows[4][2].note, Note::Off);
        assert_eq!(a.tracker_track, 0);
    }

    #[test]
    fn entry_layout_uses_page_metadata_instead_of_midi_channel_ten() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        a.set_tracker_edit(true);
        {
            let page = a.current_page_mut().unwrap();
            page.percussion = false;
            page.entry_mode = sequencer::NoteEntryMode::Manual;
            for column in &mut page.columns {
                column.channel = 9;
            }
        }
        a.tracker_track = 3;
        a.tracker_single_note(60, 100);
        assert_eq!(a.song.patterns[&0].rows[0][3].note, Note::On(60));
        ...
        a.tracker_row = 0;
        a.write_edit_notes(&[(36, 100), (38, 100)]);
        let start = a.tracker_page * LANES_PER_PAGE;
        assert_eq!(
            a.song.patterns[&0].rows[0][start..start + LANES_PER_PAGE]
                .iter()
                .filter(|cell| matches!(cell.note, Note::On(_)))
                .count(),
            2
    ...
    #[test]
    fn manual_and_one_column_cymbals_keep_normal_same_lane_storage() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        a.set_tracker_edit(true);
        ...
        a.current_page_mut().unwrap().entry_mode = sequencer::NoteEntryMode::Manual;
        a.tracker_track = 2;
        a.tracker_single_note(49, 100);
        a.tracker_single_note(36, 100);
        let start = a.tracker_page * LANES_PER_PAGE;
        assert_eq!(a.song.patterns[&0].rows[0][start + 2].note, Note::On(49));
        assert_eq!(a.song.patterns[&0].rows[1][start + 2].note, Note::On(36));

        a.current_page_mut().unwrap().entry_mode = sequencer::NoteEntryMode::OneColumn;
        a.current_page_mut().unwrap().entry_anchor = 1;
        a.tracker_row = 2;
        a.tracker_single_note(51, 100);
        a.tracker_single_note(38, 100);
        assert_eq!(a.song.patterns[&0].rows[2][start + 1].note, Note::On(51));
        assert_eq!(a.song.patterns[&0].rows[3][start + 1].note, Note::On(38));
    }

    #[test]
    fn one_column_chords_collapse_in_deterministic_gesture_order() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        a.set_tracker_edit(true);
        ...
        for note in [67, 60, 64] {
            ...
            a.tracker_gesture
                .observe(start + Duration::from_millis(1), &[0x80, note, 0]);
        }
        a.commit_tracker_gesture(start + Duration::from_millis(60));
        assert_eq!(a.song.patterns[&0].rows[0][1].note, Note::On(67));
        assert!(a.song.patterns[&0].rows[0]
            .iter()
            .take(LANES_PER_PAGE)
            .enumerate()
            .all(|(lane, cell)| lane == 1 || cell.note == Note::Empty));
        assert!(a.status.contains("chord collapsed"));
    }
    ...
    fn drum_gesture_reuses_each_voice_column_from_earlier_rows() {
        ...
        a.set_tracker_edit(true);
        a.tracker_page = a.percussion_page_index().unwrap();
        a.current_page_mut().unwrap().entry_mode = sequencer::NoteEntryMode::DrumAuto;
        a.tracker_row = 4;
        let page_start = a.tracker_page * LANES_PER_PAGE;
        let pattern = a.song.patterns.get_mut(&0).unwrap();
    ...
    fn new_kicks_and_snares_get_home_columns_and_family_continuity() {
        ...
        assert_eq!(a.song.patterns[&0].rows[2][page_start].note, Note::On(38));

        a.tracker_single_note(40, 103);
        assert_eq!(a.song.patterns[&0].rows[3][page_start].note, Note::On(40));
    }
    ...
    fn drum_gesture_preserves_occupied_cells_and_falls_to_a_free_column() {
        ...
        assert_eq!(row[1].note, Note::On(42));
        assert_eq!(row[2].note, Note::On(57));
    ...
    fn unseen_drum_does_not_consume_an_unrelated_note_off_as_fallback_space() {
        ...
        assert_eq!(
            sequencer::drum_auto_lanes(pattern, 0, a.tracker_page, &[57], &[]),
            None
        );
    ...
    fn tracker_file_screen_exposes_confirmed_pattern_clear() {
        ...
        assert_eq!(a.song.patterns[&0].rows[0][0].note, Note::On(60));
        assert!(a.confirm_pattern_clear);
        assert!(a.status.contains("1 loops detach"));
        let backend = TestBackend::new(40, 13);
        let mut terminal = Terminal::new(backend).unwrap();
        terminal
            .draw(|frame| draw_tracker_files(frame, &mut a))
            .unwrap();
        let text = terminal
            .backend()
            .buffer()
        ...
        perform(Action::Activate, &mut a, Path::new("/none"), None);
        assert_eq!(a.song.patterns[&0].rows[0][0].note, Note::Empty);
        assert!(a.song.patterns[&0].audio_loops.iter().all(Option::is_none));
    ...
    #[test]
    fn pattern_setup_keep_and_confirm_have_distinct_new_and_clear_meanings() {
        let p = presets();

        let mut keep_new = app(&p);
        keep_new.screen = Screen::TrackerFiles;
        {
            let pattern = keep_new.song.patterns.get_mut(&0).unwrap();
            pattern.meter = 3;
            pattern.rows.truncate(12);
            pattern.rows[0][0].note = Note::On(60);
        }
        keep_new.new_pattern();
        keep_new.select_pattern_meter(4);
        keep_new.pattern_setup_rows = 32;
        perform(
            Action::ClearPatternNow,
            &mut keep_new,
            Path::new("/none"),
        ...
        assert_eq!(keep_new.song.order, vec![0, 1]);
        assert_eq!(keep_new.song.patterns[&1].meter, 3);
        assert_eq!(keep_new.song.patterns[&1].rows.len(), 12);
        assert!(keep_new.song.patterns[&1]
            .rows
            .iter()
            .flatten()
            .all(|cell| { *cell == Cell::default() }));

        let mut confirm_new = app(&p);
        confirm_new.screen = Screen::TrackerFiles;
        confirm_new.new_pattern();
        confirm_new.select_pattern_meter(3);
        confirm_new.pattern_setup_rows = 24;
        perform(
            Action::ConfirmPatternClear,
            &mut confirm_new,
            Path::new("/none"),
        ...
        assert_eq!(confirm_new.song.patterns[&1].meter, 3);
        assert_eq!(confirm_new.song.patterns[&1].rows.len(), 24);

        let mut keep_clear = app(&p);
        keep_clear.screen = Screen::TrackerFiles;
        {
            let pattern = keep_clear.song.patterns.get_mut(&0).unwrap();
            pattern.meter = 3;
            pattern.rows.truncate(12);
            pattern.rows[0][0].note = Note::On(64);
        }
        keep_clear.choose_pattern_clear();
        keep_clear.select_pattern_meter(4);
        keep_clear.pattern_setup_rows = 32;
        perform(
            Action::ClearPatternNow,
            &mut keep_clear,
            Path::new("/none"),
        ...
        assert_eq!(keep_clear.song.patterns[&0].meter, 3);
        assert_eq!(keep_clear.song.patterns[&0].rows.len(), 12);
        assert_eq!(keep_clear.song.patterns[&0].rows[0][0], Cell::default());

        let mut cancelled = app(&p);
        cancelled.screen = Screen::TrackerFiles;
        let before = cancelled.song.clone();
        cancelled.new_pattern();
        perform(Action::Back, &mut cancelled, Path::new("/none"), None);
        assert_eq!(cancelled.song, before);
        assert!(!cancelled.confirm_pattern_clear);

        let mut unavailable = app(&p);
        unavailable.song.order.clear();
        unavailable.song.patterns.clear();
        unavailable.new_pattern();
        perform(
            Action::ClearPatternNow,
            &mut unavailable,
            Path::new("/none"),
    ...
    fn pattern_clipboard_pastes_new_and_over_with_confirmation() {
        ...
        let mut a = app(&p);
        a.song.patterns.get_mut(&0).unwrap().rows[0][0].note = Note::On(60);
        a.song.patterns.get_mut(&0).unwrap().audio_loops[1] = Some(sequencer::LoopSettings::new(
            "clipboard.wav".into(),
            12_000,
            sequencer::BpmInterpretation::Normal,
            0,
    ...
    #[test]
    fn tracker_mute_discards_only_its_allocated_recording_owners() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        connect_test_midi_hardware(&mut a);
        ...
        a.toggle_tracker_recording();
        a.record_tracker_midi_at(1, &[0x90, 60, 100]);
        a.record_tracker_midi_at(1, &[0x90, 64, 100]);

        assert!(a.current_page().unwrap().lanes[0].enabled);
        perform(Action::TrackerMute, &mut a, Path::new("/none"), None);
        assert!(!a.current_page().unwrap().lanes[0].enabled, "{}", a.status);
        let recording = a.tracker_recording.as_ref().unwrap();
        assert!(recording
            .active_lanes
            .values()
            .flatten()
            .all(|owner| owner.lane != 0));
        assert!(recording
            .active_lanes
            .values()
            .flatten()
            .any(|owner| owner.lane == 1));

        a.toggle_tracker_page_mute();
        let recording = a.tracker_recording.as_ref().unwrap();
        assert!(recording.active_lanes.is_empty());
        assert!(recording.lane_owners.is_empty());
        a.stop_tracker_recording();
    }

    #[test]
    fn one_column_realtime_note_off_tracks_the_current_column_owner() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        connect_test_midi_hardware(&mut a);
        ...
        a.record_tracker_midi_at(1, &[0x90, 40, 100]);
        a.record_tracker_midi_at(2, &[0x90, 43, 101]);
        a.record_tracker_midi_at(3, &[0x80, 40, 0]);
        assert_eq!(a.song.patterns[&0].rows[3][2].note, Note::Empty);
        a.record_tracker_midi_at(4, &[0x80, 43, 0]);
        assert_eq!(a.song.patterns[&0].rows[4][2].note, Note::Off);
        assert_eq!(a.tracker_track, 0);
        assert!(a
            .tracker_recording
    ...
    #[test]
    fn identical_one_column_recording_owners_require_the_final_release() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        connect_test_midi_hardware(&mut a);
        ...
        a.record_tracker_midi_at(2, &[0x90, 60, 100]);
        a.record_tracker_midi_at(3, &[0x91, 60, 101]);
        a.record_tracker_midi_at(4, &[0x80, 60, 0]);
        assert_eq!(a.song.patterns[&0].rows[4][1].note, Note::Empty);
        assert_eq!(
            a.tracker_recording.as_ref().unwrap().active_lanes[&(1, 60)].len(),
            1
        );
        a.record_tracker_midi_at(5, &[0x81, 60, 0]);
        assert_eq!(a.song.patterns[&0].rows[5][1].note, Note::Off);
        a.stop_tracker_recording();
    }

    #[test]
    fn drum_auto_realtime_quantized_hits_use_distinct_lanes_without_cursor_motion() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        connect_test_midi_hardware(&mut a);
        ...
        for note in [36, 38, 42, 49] {
            a.record_tracker_midi_at(3, &[0x92, note, 100]);
        }
        let page_start = a.tracker_page * LANES_PER_PAGE;
        assert_eq!(
            a.song.patterns[&0].rows[3][page_start..page_start + LANES_PER_PAGE]
                .iter()
                .filter(|cell| matches!(cell.note, Note::On(_)))
                .count(),
            4
        ...
        let allocated = a
            ...
            .active_lanes
            .iter()
            .map(|((_, note), owners)| (*note, owners[0].lane))
            .collect::<BTreeMap<_, _>>();
        let before = a.song.patterns[&0].rows[3].clone();
        a.record_tracker_midi_at(3, &[0x92, 45, 100]);
        assert_eq!(a.status, "DRUM LANES FULL · pattern unchanged");
        assert_eq!(a.song.patterns[&0].rows[3], before);
        for (offset, note) in [36, 38, 42, 49].into_iter().enumerate() {
    ...
    #[test]
    fn realtime_release_crosses_pattern_boundary_in_the_allocated_lane() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        connect_test_midi_hardware(&mut a);
        ...
        page.entry_mode = sequencer::NoteEntryMode::OneColumn;
        page.entry_anchor = 3;
        let next = sequencer::Pattern::empty_like_setup(8, a.current_pattern().unwrap());
        a.song.patterns.insert(1, next);
        a.song.order.push(1);
        a.toggle_tracker_recording();
        a.record_tracker_midi_at(7, &[0x90, 48, 100]);
        {
            let recording = a.tracker_recording.as_mut().unwrap();
            recording.pattern = 1;
            recording.order = 1;
        }
        a.record_tracker_midi_at(0, &[0x80, 48, 0]);
        assert_eq!(a.song.patterns[&1].rows[0][3].note, Note::Off);
        assert_eq!(a.song.patterns[&0].rows[7][3].note, Note::On(48));
        a.stop_tracker_recording();
    }

    #[test]
    fn realtime_release_wraps_pattern_loop_without_losing_lane_ownership() {
        let p = presets();
        let mut a = app(&p);
        a.screen = Screen::Tracker;
        connect_test_midi_hardware(&mut a);
        ...
        page.entry_anchor = 2;
        a.toggle_tracker_recording();
        let last = a.tracker_rows() - 1;
        a.record_tracker_midi_at(last, &[0x90, 52, 100]);
        a.record_tracker_midi_at(0, &[0x80, 52, 0]);
        assert_eq!(a.song.patterns[&0].rows[last][2].note, Note::On(52));
        assert_eq!(a.song.patterns[&0].rows[0][2].note, Note::Off);
        assert!(a
            .tracker_recording
            .as_ref()
            .unwrap()
