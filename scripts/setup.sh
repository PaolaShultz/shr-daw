...
print_setup_progress() {
  ...
  for index in "${!SETUP_PHASES[@]}"; do
    if ((index < SETUP_PHASE_INDEX)); then
      marker='done'
    elif ((index == SETUP_PHASE_INDEX)); then
      marker='partial / not complete'
    else
      marker='not started'
    fi
    printf '  [%s] %s\n' "$marker" "${SETUP_PHASES[$index]}"
  done
}

print_setup_recovery() {
  printf 'Recovery / resume:\n  rerun:'
  printf ' %q' "$0" "${SETUP_ORIGINAL_ARGS[@]}"
  printf '\n'
  if [[ -n "$RUNTIME_BACKUP" && -n "$CONTROLLER_BACKUP" ]]; then
    printf '  restore pre-wizard configuration only:\n'
    printf '    cp -p -- %q %q\n' "$RUNTIME_BACKUP" "$RUNTIME_CONFIG"
    printf '    cp -p -- %q %q\n' "$CONTROLLER_BACKUP" "$CONTROLLER_CONFIG"
  fi
  if $JACKDRC_WRITTEN; then
    if [[ -n "$JACKDRC_BACKUP" ]]; then
      printf '  restore the previous JACK command: cp -p -- %q %q\n' \
  ...
  if $FLUID_SERVICE_MASKED; then
    printf '  undo this run'\''s FluidSynth mask: systemctl --user unmask fluidsynth.service\n'
  fi
  if $AMIDIMINDER_SERVICE_MASKED; then
    printf '  undo this run'\''s amidiminder mask: sudo systemctl unmask amidiminder.service\n'
  fi
  if $PRIVATE_LOOPS_INSTALLED; then
    printf '  private loops were installed in %q; they are not removed automatically.\n' "$LOOP_INBOX"
  fi
...
setup_exit_report() {
  ...
  if ! $SETUP_FINISHED; then
    printf '\nSHR-DAW setup stopped with exit status %s.\n' "$result" >&2
    print_setup_progress >&2
    print_setup_recovery >&2
  fi
}

trap 'exit 130' INT
trap 'exit 143' TERM
trap 'setup_exit_report "$?"' EXIT
...
if [[ -t 0 ]]; then
  ...
  RUNTIME_BACKUP="$RUNTIME_CONFIG.bak-$STAMP"
  CONTROLLER_BACKUP="$CONTROLLER_CONFIG.bak-$STAMP"
fi
setup_phase_done
...
configure_exclusive_midi_services() {
  ...
  if $amidiminder_unsafe; then
    ...
    AMIDIMINDER_SERVICE_MASKED=true
